...
def extract_pdf_images(doc, page=0):
    ...
    for image in doc.getPageImageList(page):
        ...
        pix.writePNG(
            os.path.join(config.PDF_TMP_IMAGES_DIR, "page%s-%s.png" % (0, xref))
        )
        pix = None
        imagen_found_count += 1

    assert (
        imagen_found_count == 2
    ), "Se encontraron {} imagenes para codigo de seguridad".format(imagen_found_count)
...
def extract_pdf_tokens(pdf_file_path):
    ...
    for key, val in (
        ("format", "PDF 1.4"),
        ("creator", "JasperReports Library version 5.5.2"),
        ("producer", "iText 2.1.7 by 1T3XT"),
    ):
        assert (
            key in doc.metadata
        ), 'La clave "{}" no se encuentra en los metadata del PDF'.format(key)
        assert (
            val == doc.metadata[key]
        ), 'El valor de la "{}" no se igual que en los metadata del PDF'.format(key)

    page = doc.loadPage(0)

    # Test search text
    for text in [
        "Notificación de información para Código de Seguridad y Token",
        "Código Seguridad:",
        "Token:",
    ]:
        if not page.searchFor(text, hit_max=1):
            raise ValueError(
                'No se pudo encontrar la sentencia en el PDF: "{}"'.format(text)
            )
